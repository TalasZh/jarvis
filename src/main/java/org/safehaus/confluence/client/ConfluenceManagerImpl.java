...
import org.safehaus.util.RestUtil.RestException;
...
public class ConfluenceManagerImpl implements ConfluenceManager
{
    ...
    protected String get( String apiPath, Map<String, String> parameters )
            throws RestException, ConfluenceManagerException
    {
        if ( !Strings.isNullOrEmpty( username ) && !Strings.isNullOrEmpty( password ) )
        {
            return new ConfluenceRestUtil( username, password )
                    .get( String.format( "%s/%s", baseUrl, apiPath ), parameters );
        }
        else if ( JarvisContextHolder.getContext().getCookie() != null )
        {
            return new ConfluenceRestUtil( JarvisContextHolder.getContext().getCookie() )
                    .get( String.format( "%s/%s", baseUrl, apiPath ), parameters );
        }
        else
        {
    ...
    protected String post( String apiPath, ConfluenceEntity entity ) throws RestException, ConfluenceManagerException
    {
        ...
        {
            return new ConfluenceRestUtil( username, password )
                    .post( String.format( "%s/%s", baseUrl, apiPath ), entity );
        }
        else if ( JarvisContextHolder.getContext().getCookie() != null )
        {
            return new ConfluenceRestUtil( JarvisContextHolder.getContext().getCookie() )
    ...
    protected String post( String apiPath, Set<LabelResult.Label> labels )
            throws RestException, ConfluenceManagerException
    {
        if ( !Strings.isNullOrEmpty( username ) && !Strings.isNullOrEmpty( password ) )
        {
            return new ConfluenceRestUtil( username, password )
                    .post( String.format( "%s/%s", baseUrl, apiPath ), labels );
        }
        else if ( JarvisContextHolder.getContext().getCookie() != null )
        {
            return new ConfluenceRestUtil( JarvisContextHolder.getContext().getCookie() )
    ...
    protected String delete( String apiPath, Map<String, String> parameters )
            throws RestException, ConfluenceManagerException
    {
        if ( !Strings.isNullOrEmpty( username ) && !Strings.isNullOrEmpty( password ) )
        {
            return new ConfluenceRestUtil( username, password )
                    .delete( String.format( "%s/%s", baseUrl, apiPath ), parameters );
        }
        else if ( JarvisContextHolder.getContext().getCookie() != null )
        {
            return new ConfluenceRestUtil( JarvisContextHolder.getContext().getCookie() )
