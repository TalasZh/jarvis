    ...
    for (var i = 0; i < FILTERED_DATA.length; i++)
        if (FILTERED_DATA[i] != undefined && FILTERED_DATA[i] != null) {
            ...
            for (var j = 0; j < FILTERED_DATA[i].events.length; j++) {
                b.createEvent(FILTERED_DATA[i].events[j].id, i, b.vlanes[FILTERED_DATA[i].lane].position.x, getZCoordinate(FILTERED_DATA[i].events[j].eventDate) * b.vlaneInterval, FILTERED_DATA[i].events[j].eventType, FILTERED_DATA[i].type, FILTERED_DATA[i].events[j].eventInfo)
            }
        }
}
...
function createIssueLanes(data) {
    ...
    for (var j = 0; j < data.issueStatuses.length; j++) {
        if (data.issueStatuses[j].status == STATUS_OPENED) {
            ...
            if (firstEventClosed == null) {
                zPos = 0;
                length = getZCoordinate(data.issueStatuses[j].date) * b.vlaneInterval;
            }
            else {
                zPos = firstEventClosed;
                length = getZCoordinate(data.issueStatuses[j].date) * b.vlaneInterval - zPos;
        ...
        if (data.issueStatuses[j].status == STATUS_CLOSED) {
            firstEventClosed = getZCoordinate(data.issueStatuses[j].date) * b.vlaneInterval;
            hasIssues = true;
        }
    }
    ...
    if (firstEventClosed != null) {
        b.createIssue(
            ...
            b.vlanes[data.lane].position.x,
            firstEventClosed,
            20 * b.vlaneInterval - firstEventClosed,
            data.type
        );
    }
}
...
function responseToCameraEvents() {
    if (MOVE_CAMERA) {
        ...
        if (b.camera.position.z + (clickYPos - clickInitYPos) / 1000 >= b.CAMERA_POS) {
    ...
    if (val != 0) {
        if (b.zPos < 0 && val < 0) {
            ...
            b.camera.position.z = b.CAMERA_POS;
    ...
    if (animationGridMove >= b.vlaneInterval) {
        b.hlanes[b.zPos % b.hlanes.length].h.position.z = ( b.hlanes.length + b.zPos ) * b.vlaneInterval;
        b.hlanes[b.zPos % b.hlanes.length].r.position.z = ( b.hlanes.length + b.zPos ) * b.vlaneInterval;
        b.hlanes[b.zPos % b.hlanes.length].l.position.z = ( b.hlanes.length + b.zPos ) * b.vlaneInterval;
        b.hlanes[b.zPos % b.hlanes.length].hr.position.z = ( b.hlanes.length + b.zPos ) * b.vlaneInterval;
        b.hlanes[b.zPos % b.hlanes.length].hl.position.z = ( b.hlanes.length + b.zPos ) * b.vlaneInterval;

        b.insertDates(b.zPos + b.hlanes.length);

        b.zPos++;
        animationGridMove -= b.vlaneInterval;

        if (animationGridMove < 0) animationGridMove = 0;
    }

    if (animationGridMove <= -b.vlaneInterval) {
        b.hlanes[b.zPos % b.hlanes.length].h.position.z = b.zPos * b.vlaneInterval;
        b.hlanes[b.zPos % b.hlanes.length].r.position.z = b.zPos * b.vlaneInterval;
        b.hlanes[b.zPos % b.hlanes.length].l.position.z = b.zPos * b.vlaneInterval;
        b.hlanes[b.zPos % b.hlanes.length].hr.position.z = b.zPos * b.vlaneInterval;
        b.hlanes[b.zPos % b.hlanes.length].hl.position.z = b.zPos * b.vlaneInterval;
