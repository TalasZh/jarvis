...
function initScene() {
    ...
    builder = new Builder(scene, new Date("2015-09-22"), new Date("2015-09-22"));
    ...
    for (var i = 0; i < data.changelogList.length && popupLimit > 0; i++) {
        ...
        if (data.changelogList[i].changeKey.changeItemId == eventId) {
            var popupData = [
                ...
                data.issueKey, showDate, data.changelogList[i].issueKey, i, i
            ];
			createPopupWindow(popupData);
            utmost = true;
        }
...
function scrollResearch(mvmnt) {
	...
	var dateToDateFormat = new Date(selectedData.changelogList[i].changeKey.created);
	var showDate = dateToDateFormat.getDate() + '.' + dateToDateFormat.getMonth() + '.' + dateToDateFormat.getFullYear();	
    ...
    if (mvmnt > 0) {
        ...
        if (selected_doc + 4 < selectedData.changelogList.length) {
            var popupData = [
                (left * 4 - width_diff * 2), (top_offset * 4), (left + width_diff), ( top_offset - height_diff ), Z_INDEX--,
                selectedData.issueKey, showDate, selectedData.changelogList[i].issueKey, i, i
            ];
			createPopupWindow(popupData);
        }
    } else {
...
function createPopupWindow(popupData) {
