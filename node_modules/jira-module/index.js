...
JiraApi.prototype.findIssue = function(issueNumber, callback) {
	...
	this.doRequest(options, function(response){
		if(response.status === 404) {
			callback("Issue not found or you don't have permissions to view it.", response.status);
			return;
		}
		
		if(response.status !== 200) {
			callback(response.statusText + ": Unable to connect to JIRA during findIssue", response.status);
			return;
		}
		...
		callback(null, response.status, response.json);
	}, "GET");
};
...
JiraApi.prototype.transitionIssue = function(issueId, issueTransition, 
											 callback){
	var options = {
		url: this.makeUri("/issue/" + issueId + "/transitions"),
		anonymous: this.strictSSL,
		content: issueTransition
	};

	this.doRequest(options, function(response) {
		if(response.status === 404) {
			callback("The issue does not exist or the user does not have\
					 permission to view it");
			return;
		}
		if(response.status === 400) {
		...
		if(response.status !== 204) {
			callback(response.statusText + ": unable to transitionIssue");
			return;
		}
...
JiraApi.prototype.startSession = function(credentials){
	var options = {
		anonymous: this.strictSSL,
		url: this.makeUri("/session", "rest/auth/", "1"),
		content: credentials
...
JiraApi.prototype.destroySession = function(session) {
	var options = {
		anonymous: this.strictSSL,
		url: this.makeUri("/session", "/rest/auth/", "1"),
